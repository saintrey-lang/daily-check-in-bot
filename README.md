...
- In channel `1552725214868275261`, the worker posts a category dropdown for **Strategy & Tips**, **Engagement**, and **Version Discussion**. The existing Creator bot's Vercel interaction endpoint opens the modal and records submissions; Discord sends that app's interactions to its configured HTTP endpoint rather than the Gateway worker. **Task Date** defaults to today's date in Manila and can be changed to a past date using `YYYY-MM-DD`. The HTTPS link and file upload are individually optional, but at least one must be provided.
