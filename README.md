...
- In channel `1552725214868275261`, the worker posts a category dropdown for **Strategy & Tips**, **Engagement**, **Version Discussion**, and **Others**. The existing Creator bot's Vercel interaction endpoint opens the modal and records submissions; Discord sends that app's interactions to its configured HTTP endpoint rather than the Gateway worker. **Task Title** is required. **Task Date** defaults to today's date in Manila and can be changed to a past date using `YYYY-MM-DD`. The HTTPS link and file upload are individually optional, but at least one must be provided.
...
- The dashboard's **Starplayer** section shows total submissions, unique players, category counts, per-player task completion, and both the task date and actual submission time. A category counts as finished for a player after their first submission; additional submissions still increase the submission total. This is submission progress, not a review or approval status.
- Entries go to the separate [Starplayer Google Sheet](https://docs.google.com/spreadsheets/d/1aYqwNCdicmFhsITN9pXQE7shDw8xDeVT-5RUkIL3QsI/edit). Share that Sheet with `GOOGLE_SERVICE_ACCOUNT_EMAIL` as **Editor** in both the Creator bot and the dashboard/worker projects. The bot creates `StarplayerSubmissions` and `StarplayerConfig` tabs and leaves other tabs alone. It adds Task Date and Task Title to existing sheets in place, preserving earlier submissions; older rows have blank values for fields added later. Set `STARPLAYER_GOOGLE_SHEET_ID` on the worker/dashboard only if the Sheet changes.
...
6. Apply [`creator-bot-integration.patch`](creator-bot-integration.patch) to the separate Creator bot project and deploy its existing Vercel interaction endpoint. It keeps all Creator commands and adds handlers for this worker's Starplayer dropdown and modal. The Creator bot project must retain its `DISCORD_BOT_TOKEN`, `GOOGLE_SERVICE_ACCOUNT_EMAIL`, and `GOOGLE_PRIVATE_KEY` secrets. Its service account must have Editor access to the Starplayer Sheet. Leave the Discord application's Interactions Endpoint URL pointing to the Creator bot.
7. Wait for worker log `Check-in bot connected as …`. Press **Start Day 1** in the dashboard. Wait for the Day 1 prompt and your selected code in Discord. Type that code as a player: expect `DAY 1 - CHECK-IN VERIFIED`, 14 days left, and a new `Checkins` row. Test each Starplayer category with a link or file and confirm the row and dashboard count.
